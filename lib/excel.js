...
async function readExcelHoldings(filePath, category) {
  ...
  return rows
    ...
    .map(r => {
      ...
      if (r.fmvAtExercise != null && r.fmvAtExercise !== '') holding.fmvAtExercise = Number(r.fmvAtExercise);
      if (r.fmvAtVest != null && r.fmvAtVest !== '') holding.fmvAtVest = Number(r.fmvAtVest);
      return holding;
    });
}

async function readExcelLiabilities(filePath, category) {
  const rows = await readWorksheetRows(filePath);
...
async function readAllPortfolioData() {
  ...
  for (const f of files) {
    try {
      if (isLiabilityCategory(f.category, config)) {
        liabilities.push(...await readExcelLiabilities(f.path, f.category));
      } else {
        assets.push(...await readExcelHoldings(f.path, f.category));
      }
    } catch (e) {
